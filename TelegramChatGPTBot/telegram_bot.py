...
class ChatGPT3TelegramBot:
    ...
    def __init__(self, config: dict, openai: OpenAIHelper):
        ...
        self.config = config
        self.openai = openai
        self.disallowed_message = "Вибачте, не дозволено користуватись цим чатом."
    ...
    async def prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        response = self.openai.get_chat_response(
            chat_id=chat_id, query=update.message.text)
    ...
    def run(self):
        ...
        application.add_handler(CommandHandler('support', self.help))
